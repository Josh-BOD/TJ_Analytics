...
function onEdit(e) {
  ...
  // =========================================================================
  // DASHBOARD: View Level, Selection, or Metric Toggle changes
  // =========================================================================
  if (sheetName === CP_DASHBOARD_SHEET_NAME) {
    try {
      ...
      if (row === 3 && col >= 2 && col <= 6) {
        cpRefreshBreakdownTable(sheet);
        // Rebuild charts to reflect new selections
        cpRebuildDashboardCharts(sheet);
        return;
      }
      
      // Metric toggle checkbox changed (row 6, columns B-G = 2-7)
      if (row === 6 && col >= 2 && col <= 7) {
        cpRebuildDashboardCharts(sheet);
        return;
      }
    } catch (error) {
      Logger.log('Dashboard onEdit error: ' + error.toString());
    }
    return;
  }
...
function cpBuildDashboardV8() {
  ...
  try {
    ...
    // =========================================================================
    // ROW 4: Primary Date Range
    // =========================================================================
    dashSheet.getRange('A4').setValue('Date Range:');
    dashSheet.getRange('A4').setFontWeight('bold');
    ...
    dashSheet.getRange('B4').setValue(minDate);
    dashSheet.getRange('B4').setNumberFormat('yyyy-mm-dd');
    dashSheet.getRange('B4').setBackground('#e8f5e9');
    dashSheet.getRange('B4').setNote('Primary period start date');
    
    dashSheet.getRange('C4').setValue('to');
    dashSheet.getRange('C4').setHorizontalAlignment('center');
    
    dashSheet.getRange('D4').setValue(maxDate);
    dashSheet.getRange('D4').setNumberFormat('yyyy-mm-dd');
    dashSheet.getRange('D4').setBackground('#e8f5e9');
    dashSheet.getRange('D4').setNote('Primary period end date');
    
    // =========================================================================
    // ROW 5: Comparison Date Range (for period-over-period)
    // =========================================================================
    dashSheet.getRange('A5').setValue('Compare To:');
    dashSheet.getRange('A5').setFontWeight('bold');
    ...
    dashSheet.getRange('B5').setValue(compStart);
    dashSheet.getRange('B5').setNumberFormat('yyyy-mm-dd');
    dashSheet.getRange('B5').setBackground('#fff3e0');
    dashSheet.getRange('B5').setNote('Comparison period start (leave blank to disable)');
    
    dashSheet.getRange('C5').setValue('to');
    dashSheet.getRange('C5').setHorizontalAlignment('center');
    
    dashSheet.getRange('D5').setValue(compEnd);
    dashSheet.getRange('D5').setNumberFormat('yyyy-mm-dd');
    dashSheet.getRange('D5').setBackground('#fff3e0');
    dashSheet.getRange('D5').setNote('Comparison period end');
    
    dashSheet.getRange('E5').setValue('(Leave blank to disable comparison)');
    dashSheet.getRange('E5').setFontStyle('italic').setFontColor('#666666').setFontSize(9);
    
    // =========================================================================
    // ROWS 6-8: Filter Section
    // =========================================================================
    dashSheet.getRange('A6').setValue('FILTERS');
    dashSheet.getRange('A6:G6').merge();
    dashSheet.getRange('A6').setFontWeight('bold').setBackground('#ff9800').setFontColor('white');
    
    // Filter headers
    dashSheet.getRange('A7').setValue('');
    dashSheet.getRange('B7').setValue('Field');
    dashSheet.getRange('C7').setValue('Mode');
    dashSheet.getRange('D7').setValue('Value');
    dashSheet.getRange('B7:D7').setFontWeight('bold').setBackground('#ffe0b2');
    ...
    // Create 3 filter rows (rows 8, 9, 10)
    for (let i = 0; i < 3; i++) {
      const row = 8 + i;
    ...
    // Apply Filters button note
    dashSheet.getRange('G8').setValue('← Use menu "Apply Filters"');
    dashSheet.getRange('G8').setFontStyle('italic').setFontColor('#666666').setFontSize(9);
    
    // Breakdown By selector (Row 10, columns G-H)
    dashSheet.getRange('G10').setValue('Breakdown By:');
    dashSheet.getRange('G10').setFontWeight('bold');
    ...
    const breakdownByRule = SpreadsheetApp.newDataValidation()
      ...
      .setAllowInvalid(false)
      .build();
    dashSheet.getRange('H10').setDataValidation(breakdownByRule);
    dashSheet.getRange('H10').setValue('Spot Name');  // Default to Spot Name
    dashSheet.getRange('H10').setBackground('#e1bee7');  // Purple to stand out
    dashSheet.getRange('H10').setNote('Choose how to group the breakdown table');
    
    // =========================================================================
    // ROW 11-12: Metric Toggle - Labels on row 11, Checkboxes on row 12
    // =========================================================================
    dashSheet.getRange('A11').setValue('Metrics:');
    dashSheet.getRange('A11').setFontWeight('bold');
    dashSheet.getRange('A12').setValue('Show:');
    dashSheet.getRange('A12').setFontWeight('bold');
    
    dashSheet.getRange('B11:H12').clearDataValidations();
    ...
    for (let i = 0; i < metricNames.length; i++) {
      const col = 2 + i;
      dashSheet.getRange(11, col).setValue(metricNames[i]);
      dashSheet.getRange(11, col).setFontWeight('bold').setHorizontalAlignment('center');
      const checkCell = dashSheet.getRange(12, col);
    ...
    // =========================================================================
    // ROW 14: KPI Summary Cards
    // =========================================================================
    dashSheet.getRange('A14').setValue('SUMMARY (Selection 1 - Primary Period)');
    dashSheet.getRange('A14:H14').merge();
    dashSheet.getRange('A14').setFontWeight('bold').setBackground('#1a73e8').setFontColor('white');
    
    const kpiHeaders = ['Total Spend', 'Conversions', 'CPA', 'eCPM', 'Impressions', 'Clicks', 'CTR', 'Avg eCPM'];
    dashSheet.getRange('A15:H15').setValues([kpiHeaders]);
    dashSheet.getRange('A15:H15').setFontWeight('bold').setBackground('#e3f2fd');
    ...
    // KPI row 16 (values)
    dashSheet.getRange('A16').setFormula(buildSumFormula('M', '$AA$4'));  // Spend
    dashSheet.getRange('B16').setFormula(buildSumFormula('L', '$AA$4'));  // Conversions
    dashSheet.getRange('C16').setFormula('=IF(B16>0,A16/B16,0)');  // CPA
    dashSheet.getRange('D16').setFormula('=IF(E16>0,(A16/E16)*1000,0)');  // eCPM
    dashSheet.getRange('E16').setFormula(buildSumFormula('J', '$AA$4'));  // Impressions
    dashSheet.getRange('F16').setFormula(buildSumFormula('K', '$AA$4'));  // Clicks
    dashSheet.getRange('G16').setFormula('=IF(E16>0,F16/E16*100,0)');  // CTR
    
    // Format KPI values
    dashSheet.getRange('A16').setNumberFormat('$#,##0.00');
    dashSheet.getRange('B16').setNumberFormat('#,##0');
    dashSheet.getRange('C16').setNumberFormat('$#,##0.00');
    dashSheet.getRange('D16').setNumberFormat('$#,##0.000');
    dashSheet.getRange('E16').setNumberFormat('#,##0');
    dashSheet.getRange('F16').setNumberFormat('#,##0');
    dashSheet.getRange('G16').setNumberFormat('0.00"%"');
    dashSheet.getRange('A16:G16').setFontSize(14).setFontWeight('bold');
    
    // =========================================================================
    // ROW 18: Comparison Period Summary (if enabled)
    // =========================================================================
    dashSheet.getRange('A18').setValue('COMPARISON PERIOD (vs Above)');
    dashSheet.getRange('A18:G18').merge();
    dashSheet.getRange('A18').setFontWeight('bold').setBackground('#ff9800').setFontColor('white');
    
    dashSheet.getRange('A19:G19').setValues([['Spend', 'Conv', 'CPA', 'Δ Spend', 'Δ Conv', 'Δ CPA', 'Status']]);
    dashSheet.getRange('A19:G19').setFontWeight('bold').setBackground('#ffe0b2');
    
    // Row 20: Comparison values (placeholder - populated by Apply Filters)
    dashSheet.getRange('A20').setValue('(Run "Apply Filters" to see comparison)');
    dashSheet.getRange('A20:G20').merge();
    dashSheet.getRange('A20').setFontStyle('italic').setFontColor('#666666');
    ...
    dashSheet.getRange('AA11').setFormula('=B4');
    dashSheet.getRange('AA12').setValue('Primary End');
    dashSheet.getRange('AA13').setFormula('=D4');
    dashSheet.getRange('AA14').setValue('Comp Start');
    dashSheet.getRange('AA15').setFormula('=B5');
    dashSheet.getRange('AA16').setValue('Comp End');
    dashSheet.getRange('AA17').setFormula('=D5');
    // Filter values (stored when Apply Filters is run)
    dashSheet.getRange('AA20').setValue('Filter1 Field');
    dashSheet.getRange('AA21').setFormula('=B8');
    dashSheet.getRange('AA22').setValue('Filter1 Mode');
    dashSheet.getRange('AA23').setFormula('=C8');
    dashSheet.getRange('AA24').setValue('Filter1 Value');
    dashSheet.getRange('AA25').setFormula('=D8');
    dashSheet.getRange('AA26').setValue('Filter2 Field');
    dashSheet.getRange('AA27').setFormula('=B9');
    dashSheet.getRange('AA28').setValue('Filter2 Mode');
    dashSheet.getRange('AA29').setFormula('=C9');
    dashSheet.getRange('AA30').setValue('Filter2 Value');
    dashSheet.getRange('AA31').setFormula('=D9');
    dashSheet.getRange('AA32').setValue('Filter3 Field');
    dashSheet.getRange('AA33').setFormula('=B10');
    dashSheet.getRange('AA34').setValue('Filter3 Mode');
    dashSheet.getRange('AA35').setFormula('=C10');
    dashSheet.getRange('AA36').setValue('Filter3 Value');
    dashSheet.getRange('AA37').setFormula('=D10');
    // Breakdown By setting
    dashSheet.getRange('AA38').setValue('Breakdown By');
    dashSheet.getRange('AA39').setFormula('=H10');
    
    // Hide column AA
    dashSheet.hideColumns(27);
    
    // =========================================================================
    // ROW 22+: TREND DATA (header row)
    // =========================================================================
    dashSheet.getRange('A22').setValue('TREND DATA');
    dashSheet.getRange('A22').setFontWeight('bold').setBackground('#1a73e8').setFontColor('white');
    ...
    dashSheet.getRange(23, 1, 1, trendHeaders.length).setValues([trendHeaders]);
    dashSheet.getRange(23, 1, 1, trendHeaders.length).setFontWeight('bold').setBackground('#e3f2fd').setFontSize(9);
    ...
    const dataStartRow = 24;  // Row after headers
    const trendRows = Math.min(dates.length, 14);
...
function cpBuildDashboardChartsV8(dashSheet, chartRows) {
  ...
  const toggles = dashSheet.getRange('B12:G12').getValues()[0];
  const showSpend = toggles[0] === true;
  const showCPA = toggles[1] === true;
  const showConv = toggles[2] === true;
  const showImpr = toggles[3] === true;
  ...
  for (const metric of allMetrics) {
    ...
    for (let s = 0; s < 5; s++) {
      ...
      const activeSel = activeSelections.find(a => a.index === s);
      if (activeSel) {
        dashSheet.getRange(23, col).setValue(`${activeSel.shortName} ${metric}`);
      } else {
        dashSheet.getRange(23, col).setValue('');
      }
    }
  }
  ...
  if (showSpend || showConv || showCPA) {
    const chart1Builder = dashSheet.newChart()
      .setChartType(Charts.ChartType.COMBO)
      .addRange(dashSheet.getRange(23, 1, chartRows, 1))
      .setNumHeaders(1)
      .setPosition(1, 9, 0, 0)
      .setOption('title', 'Spend, Conversions & CPA')
      .setOption('width', 700)
    ...
    if (showSpend) {
      for (const sel of activeSelections) {
        const col = metricColStarts['Spend'] + sel.index;
        chart1Builder.addRange(dashSheet.getRange(23, col, chartRows, 1));
        seriesConfig[seriesIndex++] = { 
  ...
  if (showImpr || showClicks || showCTR) {
    ...
    if (showImpr) {
      for (const sel of activeSelections) {
        const col = metricColStarts['Impr'] + sel.index;
        chart2Builder.addRange(dashSheet.getRange(23, col, chartRows, 1));
        seriesConfig2[seriesIndex2++] = { 
          color: colorPalette[sel.index].light, lineWidth: 1, pointSize: 5,
  ...
  if (activeSelections.length > 0) {
    const chart3Builder = dashSheet.newChart()
      ...
      .addRange(dashSheet.getRange(23, 1, chartRows, 1))  // Date column
      .setNumHeaders(1)
    ...
    for (const sel of activeSelections) {
      ...
      chart3Builder.addRange(dashSheet.getRange(23, col, chartRows, 1));
...
function cpApplyDashboardFilters() {
  ...
  try {
    ...
    // Read date ranges
    const primaryStart = dashSheet.getRange('B4').getValue();
    const primaryEnd = dashSheet.getRange('D4').getValue();
    const compStart = dashSheet.getRange('B5').getValue();
    const compEnd = dashSheet.getRange('D5').getValue();
    ...
    // Update comparison section (row 20)
    if (hasComparison) {
      ...
      dashSheet.getRange('A20:G20').breakApart();
      dashSheet.getRange('A20:G20').setValues([[
        compMetrics.spend,
        compMetrics.conv,
        compMetrics.cpa,
      ...
      dashSheet.getRange('A20').setNumberFormat('$#,##0.00');
      dashSheet.getRange('B20').setNumberFormat('#,##0');
      dashSheet.getRange('C20').setNumberFormat('$#,##0.00');
      
      // Color the delta cells
      const spendCell = dashSheet.getRange('D20');
      const convCell = dashSheet.getRange('E20');
      const cpaCell = dashSheet.getRange('F20');
      ...
      dashSheet.getRange('A20:G20').breakApart();
      dashSheet.getRange('A20').setValue('(No comparison period set)');
      dashSheet.getRange('A20:G20').merge();
      dashSheet.getRange('A20').setFontStyle('italic').setFontColor('#666666');
    }
    ...
    const lastDataRow = dashSheet.getLastRow();
    let dataRowCount = 0;
    for (let r = 24; r <= Math.min(lastDataRow, 40); r++) {
    ...
    // =========================================================================
    const selection1 = dashSheet.getRange('B3').getValue();
    const breakdownBy = dashSheet.getRange('H10').getValue() || 'Spot Name';  // Read from Breakdown By dropdown
    const breakdownStartRow = 65;
    ...
    if (selection1 === '(All)' && breakdownBy) {
      ...
      dashSheet.getRange(breakdownDataStart, 1).setValue('Select a "Breakdown By" option (H10) to see breakdown');
