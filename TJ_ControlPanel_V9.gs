/**
 * TJ Control Panel V9 - Google Apps Script
 * 
 ...
 * 
 * V9 CHANGES:
 * - Enhanced Pivot Table: Comprehensive metrics (T/Y/7D for eCPM, Spend, CPA, Conv, CTR)
 * - Extended Edit Columns: T Bid Adjust, Date last bid Adjust, Spot ID, Geo ID, Last Updated
 * - Refresh Behavior: Clear all edit data on refresh (pivot accordion changes misalign rows)
 * - Filters in Pivot: Country and other dimensions via native pivot filters
 * 
 * V8 FEATURES:
...
function onOpen() {
  ...
  ui.createMenu('Control Panel')
    ...
    .addSubMenu(ui.createMenu('📊 Dashboard (V9)')
...
function cpPreparePivotSource() {
  ...
  try {
    ...
    ui.alert('Pivot Data Ready!', 
      ...
      `2. Go to Insert > Pivot table > New sheet\n` +
      `3. In the Pivot table editor, add:\n\n` +
      `ROWS (in this order):\n` +
      `   • Tier 1 Strategy\n` +
      `   • Sub Strategy\n` +
      `   • Campaign Name\n` +
      `   • Spot Name\n\n` +
      `VALUES (choose aggregation):\n` +
      `   • Current eCPM Bid (AVG)\n` +
      `   • Daily Budget (SUM)\n` +
      `   • T/Y/7D eCPM (AVG each)\n` +
      `   • T/Y/7D Spend (SUM each)\n` +
      `   • T/Y/7D CPA (AVG each)\n` +
      `   • T/Y/7D Conv (SUM each)\n` +
      `   • T/Y/7D CTR (AVG each)\n` +
      `   • 7D Active Days (AVG)\n\n` +
      `FILTERS (optional):\n` +
      `   • Country\n\n` +
      `4. Rename sheet to "${CP_PIVOT_SHEET_NAME}"\n` +
      `5. Run "Add Edit Columns to Pivot"`,
      ui.ButtonSet.OK);
...
/**
 * STEP 2: Add Edit Columns to an existing Pivot Table (V9 Enhanced)
 * 
 * Detects your manually-created pivot table and adds:
 * - 📊 Checkbox (Dashboard navigation)
 * - New CPM column (editable)
 * - New Budget column (editable)
 * - Comment column (editable)
 * - T Bid Adjust (lookup formula)
 * - Date last bid Adjust (lookup formula)
 * - Bid ID, Campaign ID, Spot ID, Geo ID (lookup formulas)
 * - Last Updated (lookup formula)
 */
function cpAddEditColumnsToPivot() {
  ...
  try {
    ...
    // V9 Enhanced: 12 edit columns total
    // Order: Spacer → Checkbox → New CPM → New Budget → Comment → T Bid Adjust → Date last bid Adjust → Bid ID → Campaign ID → Spot ID → Geo ID → Last Updated
    const spacerCol = pivotLastCol + 1;
    const checkboxCol = pivotLastCol + 2;        // Dashboard navigation
    ...
    const tBidAdjustCol = pivotLastCol + 6;      // Lookup - Column M
    const dateBidAdjustCol = pivotLastCol + 7;   // Lookup - Column N
    const bidIdCol = pivotLastCol + 8;           // Lookup - Column AH
    const campIdCol = pivotLastCol + 9;          // Lookup - Column D
    const spotIdCol = pivotLastCol + 10;         // Lookup - Column AG
    const geoIdCol = pivotLastCol + 11;          // Lookup - Column AI
    const lastUpdatedCol = pivotLastCol + 12;    // Lookup - Column AJ
    
    const totalEditCols = 12;
    ...
    if (pivotSheet.getMaxColumns() > pivotLastCol) {
      ...
      const existingHeaders = pivotSheet.getRange(1, pivotLastCol + 1, 1, 15).getValues()[0];
      const hasEditCols = existingHeaders.includes('Bid ID') || existingHeaders.includes('New CPM') || existingHeaders.includes('📊');
      if (hasEditCols) {
        // Clear existing edit columns - V9 clears ALL data including edits
        pivotSheet.getRange(1, pivotLastCol + 1, formulaRowCount + 10, 15).clear();
        pivotSheet.getRange(2, pivotLastCol + 1, formulaRowCount + 10, 15).clearDataValidations();
        Logger.log('Cleared existing edit columns (V9: full clear including edits)');
      }
    }
    
    // V9 Headers for edit columns (12 total)
    const editHeaders = ['', '📊', 'New CPM', 'New Budget', 'Comment', 'T Bid Adj', 'Date Bid Adj', 'Bid ID', 'Campaign ID', 'Spot ID', 'Geo ID', 'Last Updated'];
    pivotSheet.getRange(1, spacerCol, 1, editHeaders.length).setValues([editHeaders]);
    
    // Format editable column headers (Checkbox, New CPM, New Budget, Comment) - Orange
    pivotSheet.getRange(1, checkboxCol, 1, 4)
      .setFontWeight('bold')
      .setBackground('#ff9800')
      .setFontColor('white');
    
    // Format lookup column headers (T Bid Adj, Date Bid Adj) - Blue
    pivotSheet.getRange(1, tBidAdjustCol, 1, 2)
      .setFontWeight('bold')
      .setBackground('#2196f3')
      .setFontColor('white');
    
    // Format ID column headers (Bid ID, Campaign ID, Spot ID, Geo ID, Last Updated) - Gray
    ...
    // Build lookup formulas for each row - V9 now includes 7 lookup columns
    // Control Panel columns: M=T Bid Adjust, N=Date last bid Adjust, D=Campaign ID, AG=Spot ID, AH=Bid ID, AI=Geo ID, AJ=Last Updated
    if (formulaRowCount > 1) {
      ...
      // Common MATCH formula for all lookups
      const matchFormula = (row) => `MATCH(1,('${CP_SHEET_NAME}'!$A$3:$A=${sCol}${row})*('${CP_SHEET_NAME}'!$B$3:$B=${ssCol}${row})*('${CP_SHEET_NAME}'!$C$3:$C=${cCol}${row})*('${CP_SHEET_NAME}'!$H$3:$H=${spCol}${row}),0)`;
      
      for (let row = 2; row <= formulaRowCount; row++) {
        // T Bid Adjust - Column M
        const tBidAdjFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$M$3:$M,${matchFormula(row)}),"")`;
        
        // Date last bid Adjust - Column N
        const dateBidAdjFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$N$3:$N,${matchFormula(row)}),"")`;
        
        // Bid ID - Column AH
        const bidFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$AH$3:$AH,${matchFormula(row)}),"")`;
        
        // Campaign ID - Column D
        const campFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$D$3:$D,${matchFormula(row)}),"")`;
        
        // Spot ID - Column AG
        const spotFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$AG$3:$AG,${matchFormula(row)}),"")`;
        
        // Geo ID - Column AI
        const geoFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$AI$3:$AI,${matchFormula(row)}),"")`;
        
        // Last Updated - Column AJ
        const lastUpdatedFormula = `=IFERROR(INDEX('${CP_SHEET_NAME}'!$AJ$3:$AJ,${matchFormula(row)}),"")`;
        
        formulas.push([tBidAdjFormula, dateBidAdjFormula, bidFormula, campFormula, spotFormula, geoFormula, lastUpdatedFormula]);
      }
      
      // Set all formulas at once (batch operation) - 7 columns starting at tBidAdjustCol
      pivotSheet.getRange(2, tBidAdjustCol, formulaRowCount - 1, 7).setFormulas(formulas);
      Logger.log(`Added ${formulas.length} rows of lookup formulas (7 columns each)`);
    }
    ...
    // Light blue for lookup info columns (T Bid Adj, Date Bid Adj)
    pivotSheet.getRange(2, tBidAdjustCol, formulaRowCount - 1, 2).setBackground('#e3f2fd');
    
    // Gray text for ID columns (reference only - at the end)
    pivotSheet.getRange(2, bidIdCol, formulaRowCount - 1, 5).setFontColor('#999999');
    ...
    pivotSheet.getRange(2, tBidAdjustCol, formulaRowCount - 1, 1).setNumberFormat('0.0"%"');
    pivotSheet.getRange(2, dateBidAdjustCol, formulaRowCount - 1, 1).setNumberFormat('yyyy-mm-dd');
    pivotSheet.getRange(2, lastUpdatedCol, formulaRowCount - 1, 1).setNumberFormat('yyyy-mm-dd hh:mm');
    
    // Auto-resize edit columns
    pivotSheet.autoResizeColumns(checkboxCol, totalEditCols - 1);
    
    // Set checkbox column width (narrower)
    pivotSheet.setColumnWidth(checkboxCol, 30);
    
    ui.alert('Edit Columns Added (V9)!', 
      `Added ${totalEditCols} edit columns to your pivot table:\n\n` +
      `EDITABLE (yellow):\n` +
      `• 📊 - Click to view on Dashboard\n` +
      `• New CPM - Enter new bid values\n` +
      `• New Budget - Enter new budget values\n` +
      `• Comment - Optional notes\n\n` +
      `INFO (blue):\n` +
      `• T Bid Adj - Today's bid adjustment %\n` +
      `• Date Bid Adj - Last bid change date\n\n` +
      `IDs (gray):\n` +
      `• Bid ID, Campaign ID, Spot ID, Geo ID, Last Updated\n\n` +
      `To update bids/budgets:\n` +
      `1. Enter values in New CPM or New Budget\n` +
...
/**
 * Refresh the edit columns on an existing pivot table (V9 Enhanced)
 * 
 * V9 BEHAVIOR: CLEARS ALL edit data and regenerates everything.
 * This is necessary because when the pivot accordion changes (expand/collapse),
 * the rows shift and edit data becomes misaligned.
 * 
 * What this does:
 * - Clears New CPM, New Budget, Comment values
 * - Clears and regenerates all lookup formulas
 * - Resets checkboxes
 * 
 * Use after refreshing Control Panel data or changing pivot structure.
 */
function cpRefreshPivotEditColumns() {
  ...
  try {
    // Find the edit columns by looking for "📊" header (checkbox column)
    ...
    // V9 column layout (12 total from checkbox)
    const newCpmCol = checkboxCol + 1;
    const newBudgetCol = checkboxCol + 2;
    const commentCol = checkboxCol + 3;
    const tBidAdjustCol = checkboxCol + 4;
    const dateBidAdjustCol = checkboxCol + 5;
    const bidIdCol = checkboxCol + 6;
    const campIdCol = checkboxCol + 7;
    const spotIdCol = checkboxCol + 8;
    const geoIdCol = checkboxCol + 9;
    const lastUpdatedCol = checkboxCol + 10;
    ...
    Logger.log(`V9 Refresh: Clearing and regenerating ALL edit columns for ${formulaRowCount} rows...`);
    
    // V9: CLEAR ALL edit data (not just formulas)
    // Clear editable columns (New CPM, New Budget, Comment)
    pivotSheet.getRange(2, newCpmCol, formulaRowCount - 1, 3).clearContent();
    
    // Clear checkboxes
    pivotSheet.getRange(2, checkboxCol, formulaRowCount - 1, 1).clearContent();
    pivotSheet.getRange(2, checkboxCol, formulaRowCount - 1, 1).clearDataValidations();
    
    // Re-add checkbox validation
    const checkboxRule = SpreadsheetApp.newDataValidation()
      .requireCheckbox()
      .setAllowInvalid(false)
      .build();
    ...
    // Generate and set ALL lookup formulas (7 columns)
    const formulas = [];
    const sCol = String.fromCharCode(64 + strategyCol);
    const ssCol = String.fromCharCode(64 + subStratCol);
    const cCol = String.fromCharCode(64 + campaignCol);
    ...
    // Set all lookup formulas (7 columns starting at tBidAdjustCol)
    pivotSheet.getRange(2, tBidAdjustCol, formulaRowCount - 1, 7).setFormulas(formulas);
    ...
    ui.alert('Refreshed (V9)!', 
      `Cleared and regenerated ALL edit columns:\n\n` +
      `• Cleared: New CPM, New Budget, Comment\n` +
      `• Reset: Checkboxes\n` +
      `• Regenerated: ${formulas.length} rows of lookup formulas\n\n` +
      `NOTE: Edit data was cleared because pivot row order may have changed.`,
      ui.ButtonSet.OK);
...
// V9 order: Checkbox → New CPM → New Budget → Comment → T Bid Adj → Date Bid Adj → Bid ID → Campaign ID → Spot ID → Geo ID → Last Updated
const PIVOT_EDIT_COLS = {
  ...
  T_BID_ADJ_OFFSET: 6,      // pivotLastCol + 6
  DATE_BID_ADJ_OFFSET: 7,   // pivotLastCol + 7
  BID_ID_OFFSET: 8,         // pivotLastCol + 8
  CAMP_ID_OFFSET: 9,        // pivotLastCol + 9
  SPOT_ID_OFFSET: 10,       // pivotLastCol + 10
  GEO_ID_OFFSET: 11,        // pivotLastCol + 11
  LAST_UPDATED_OFFSET: 12   // pivotLastCol + 12
};

// Helper to find edit column positions in pivot sheet (V9 Enhanced)
function getPivotEditColumns(sheet) {
  const lastCol = sheet.getLastColumn();
  const headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
  
  // Find columns by header name (dynamic detection)
  let checkboxCol = -1, newCpmCol = -1, newBudgetCol = -1, commentCol = -1;
  let tBidAdjCol = -1, dateBidAdjCol = -1;
  let bidIdCol = -1, campIdCol = -1, spotIdCol = -1, geoIdCol = -1, lastUpdatedCol = -1;
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    else if (h === 'T Bid Adj') tBidAdjCol = i + 1;
    else if (h === 'Date Bid Adj') dateBidAdjCol = i + 1;
    else if (h === 'Bid ID') bidIdCol = i + 1;
    else if (h === 'Campaign ID') campIdCol = i + 1;
    else if (h === 'Spot ID') spotIdCol = i + 1;
    else if (h === 'Geo ID') geoIdCol = i + 1;
    else if (h === 'Last Updated') lastUpdatedCol = i + 1;
  }
  ...
  return {
    ...
    tBidAdjCol: tBidAdjCol,
    dateBidAdjCol: dateBidAdjCol,
    bidIdCol: bidIdCol,
    campIdCol: campIdCol,
    spotIdCol: spotIdCol,
    geoIdCol: geoIdCol,
