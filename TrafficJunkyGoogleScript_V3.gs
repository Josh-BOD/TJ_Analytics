...
function fetchPostHogConversions(startDate, endDate) {
  ...
  if (daysDiff > 1) {
    Logger.log(`Fetching ${daysDiff} days of data day-by-day to avoid timeout...`);
...
/**
 * Fetch PostHog conversions day by day to avoid timeout errors
 */
function fetchPostHogConversionsByDay(startDate, endDate) {
  ...
  while (currentDate <= endDate) {
    ...
    Logger.log(`Fetching PostHog data for ${dateStr} (day ${daysProcessed})...`);
    ...
    try {
      ...
      // Append results
      if (dayData.results && Array.isArray(dayData.results)) {
        allResults.push(...dayData.results);
        totalFetched += dayData.results.length;
        Logger.log(`  Found ${dayData.results.length} conversions for ${dateStr}`);
      }
      
    } catch (error) {
      Logger.log(`  Warning: Error fetching ${dateStr}: ${error.toString()}`);
  ...
  Logger.log(`PostHog day-by-day fetch complete: ${totalFetched} total conversions from ${daysProcessed} days`);
  
  // Return combined results in same format as single query
  return {
    columns: columns || [],
    results: allResults
  };
