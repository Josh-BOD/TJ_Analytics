...
function buildPostHogQuery(startDateStr, endDateStr) {
  // OPTIMIZED: Date and TrafficJunky filters moved INSIDE the CTE to reduce data before ROW_NUMBER
  // This dramatically improves performance by filtering early
  const query = `
SELECT
...
function fetchPostHogConversionsByDay(startDate, endDate) {
  Logger.log(`=== Day-by-Day Fetch Starting ===`);
  ...
  while (currentDate <= endDate) {
    ...
    try {
      ...
      if (dayData.results && Array.isArray(dayData.results)) {
        ...
        Logger.log(`  ✓ Found ${dayData.results.length} conversions for ${dateStr} (running total: ${totalFetched})`);
      } else {
